...
const readFile = (filename) => new Promise((resolve, reject) => {
    if (Meteor.isServer) {
        ...
        //const filepath = Assets.absoluteFilePath(filename);// Assets can only access files in private directory, Error: Unknown asset: cjklib/characterdecomposition.csv
        //const filepath = path.join(getPWD(), filename); //public目录下的文件可以直接访问，http://localhost:3000/gb2312，但是服务端不能这样访问Meteor.absoluteUrl(filename)
        const filepath = path.join(getPWD(), 'local/build/programs/web.browser/app', filename); //地址对了但仍然报错
        //const filepath = Assets.absoluteFilePath('public/' + filename);//最好是把public下的这些数据文件放在private目录下
        //console.log('readFile filepath:' + filepath);
        fs.readFile(filepath, 'utf8', (error, data) => {
            //throw new Error('Error filepath:' + filepath); // show the abs path
...
Meteor.startup(() => {
    ...
    //fs.symlinkSync('../../../../public', '.meteor/local/build/programs/web.browser/app');
